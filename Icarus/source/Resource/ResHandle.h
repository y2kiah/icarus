...
typedef weak_ptr<ResCacheManager>	ResCacheManagerWeakPtr;
...
class ResHandle : private boost::noncopyable {
	...
	protected:
		static ResCacheManagerWeakPtr	sResCacheManager;	// injected by ResCacheManager when it is created

		wstring	mName;		// this is the resource name, could be a filename or application-assigned
		wstring	mSource;	// this is the source name, could be a filename or application-assigned
	...
	public:
		...
		inline bool load(const wstring &resPath);
		...
		---------------------------------------------------------------------*/
		template <typename TResource>
		inline ResLoadResult tryLoad(const wstring &resPath);
		...
		---------------------------------------------------------------------*/
		bool getFromCache(const wstring &resName, ResCacheType cacheType);

		// Accessors
		const wstring &	name() const		{ return mName; }
		const wstring &	source() const		{ return mSource; }
...
class Resource : private boost::noncopyable {
	...
	public:
		...
		///// DEFINITIONS /////
		typedef shared_ptr<char>		CharBufferPtr;

		///// FUNCTIONS /////
		// Accessors
		const wstring &	name() const	{ return mName; }
		size_t			sizeB() const	{ return mSizeB; }
		...
		// Mutators
		void setSizeB(size_t sizeB) { mSizeB = sizeB; }
		...
		---------------------------------------------------------------------*/
		inline void setResCache(const ResCachePtr &resCachePtr);
		...
		/*---------------------------------------------------------------------
			...
			To support full initialization for both synchronous and async
			patterns, it is useful to write as much init code in onThreadInit
			as safe and possible, and call onThreadInit from onLoad if async is
			false. That way the full initialization code path is run by either
			pattern, without writing duplicate code.
		---------------------------------------------------------------------*/
		...
		/*---------------------------------------------------------------------
			...
			from disk, and before onLoad is run on the main thread. For example
			a D3D11 resource may want to initialize here using the ID3D11Device
			which is thread-safe. If a DeviceContext is also needed, it should
			do those init steps in the onLoad method where the immediate
			context can safely be accessed on the main thread.
		---------------------------------------------------------------------*/
		...
		---------------------------------------------------------------------*/
		explicit Resource(const wstring &name, size_t sizeB, const ResCachePtr &resCachePtr) :
