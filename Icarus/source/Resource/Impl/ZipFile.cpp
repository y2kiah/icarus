...
Description: The declaration of a quick'n dirty ZIP file reader class. Original code from Javier Arevalo.
...
optional<int> ZipFile::find(const wstring &path) const
{
	wstring lwrPath(path);
	std::transform(lwrPath.begin(), lwrPath.end(), lwrPath.begin(), ::tolower);

	ZipContentsMap::const_iterator i = mZipContentsMap.find(lwrPath);
	if (i == mZipContentsMap.end()) {
		debugWPrintf(L"ZipFile: find(\"%s\") file not found!\n", path.c_str());
		return optional<int>();
	}
	return (*i).second;
}
...
void ZipFile::close()
{
	...
	if (mInitFlags[INIT_OPEN]) {
		...
		for (uint32_t f = 0; f < mFile.size(); ++f) {
			fclose(mFile[f]);
		}
	}
}
...
/*---------------------------------------------------------------------
	...
	error.
---------------------------------------------------------------------*/
size_t ZipFile::getResourceSize(const wstring &resName) const
{
	optional<int> resNum = find(resName);
	if (resNum) {
...
---------------------------------------------------------------------*/
size_t ZipFile::getResource(const wstring &resName, CharBufferPtr &dataPtr, size_t threadIndex)
{
	optional<int> resNum = find(resName);
	if (resNum) {
		size_t size = getFileLen(*resNum);
...
---------------------------------------------------------------------*/
size_t ZipFile::getFileLen(int i) const
{
	if (i < 0 || i >= mEntries) {
...
---------------------------------------------------------------------*/
bool ZipFile::readFile(int i, void *pBuf, size_t threadIndex)
{
...
---------------------------------------------------------------------*/
size_t ZipFile::getNewThreadIndex()
{
	mFile.push_back(0);
	size_t index = mFile.size() - 1;
